...
def process_genre_labels(songGenres, allGenreIds, num_of_songs, num_of_genres):
...
# Process genre ID list of strings into a binary feature vector
songGenreCode = process_genre_labels(songGenres, allGenreIds, num_of_songs, num_of_genres)
...
def model(X, w_h, b_h, w_h2, b_h2, w_o, b_o, p_keep_input, p_keep_hidden):
    X = tf.nn.dropout(X, p_keep_input) # training using dropout
    h = tf.nn.relu(tf.matmul(X, w_h) + b_h) # input layer with relu

    h = tf.nn.dropout(h, p_keep_hidden) # with dropout
    h2 = tf.nn.relu(tf.matmul(h, w_h2) + b_h2) # hidden layer with relu

    return tf.nn.sigmoid(tf.matmul(h2, w_o) + b_o) # note that we dont take the softmax because it does not work for multi label problems
...
networksToTry = np.power(2, (np.arange(9.) + 1)) # num of neurons in hidden layer in each network
...
for k in range(0, len(networksToTry)):
    neuronsInputLayer = 518
    numOfNeurons_hidden = int(networksToTry[k]) # num of neurons in input layer
    w_h = init_weights([num_of_features, neuronsInputLayer]) # create symbolic variables
    b_h = init_weights([neuronsInputLayer])
    w_h2 = init_weights([neuronsInputLayer, numOfNeurons_hidden]) # create symbolic variables
    b_h2 = init_weights([numOfNeurons_hidden])
    w_o = init_weights([numOfNeurons_hidden, num_of_genres])
    ...
    with tf.Session() as sess:
        ...
        for i in range(epochs): # use this for loop to do batch training in epochs
            ...
            for j in range(num_of_batches):
                ...
                ptr+=batch_size
                sess.run(train_op, feed_dict={X: inp, Y: out, p_keep_input: 0.8, p_keep_hidden: 0.5}) # train network
                prY = sess.run(predict_op, feed_dict={X: teX, p_keep_input: 1.0, p_keep_hidden: 1.0}) # get predictions
...
np.savetxt('Results/accMetric_DeepNN.csv', accMetric, delimiter=",")

# Save the number of neurons in the input layer
np.savetxt('Results/numberOfNeurons_DeepNN.csv', networksToTry, delimiter=",")
...
plt.savefig('Figures/DeepNN_topGenre.png')
plt.close()
