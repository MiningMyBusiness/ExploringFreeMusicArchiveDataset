...
fullAccMetric = np.zeros([num_of_ks, onlyFeats[(num_of_training+1):].shape[0]]) # create a variable to store one entire set of accuracy metrics for each k-NN
numOfTrueLabels = np.zeros([num_of_ks, onlyFeats[(num_of_training+1):].shape[0]]) # variable that will store the number of true genre labels for a test song
numOfPredLabels = np.zeros([num_of_ks, onlyFeats[(num_of_training+1):].shape[0]]) # variable that will store the number of predicted genre labels for a test song
...
for j in range(0,num_of_ks):
    ...
    for testNum in range(0,num_of_tests):
        ...
        onlyFeats_norm = onlyFeats_norm[randOrder] # order sets randomly
        songGenreCode = songGenreCode[randOrder]
        trX = onlyFeats_norm[0:num_of_training,:] # break data into training and testing sets
        trY = songGenreCode[0:num_of_training,:]
        teX = onlyFeats_norm[(num_of_training+1):,:]
...
np.savetxt('Results/JaccMetric_kNN.csv', accMetric, delimiter=",")

# Save accuracy metric for every test point for every k-NN for one trial
np.savetxt('Results/fullJaccMetric_kNN.csv', fullAccMetric, delimiter=",")

# Save number of true labels for every test point for every k-NN for one trial
np.savetxt('Results/numOfTrueLabels_kNN.csv', numOfTrueLabels, delimiter=",")

# Save number of predicted labels for every test point...
np.savetxt('Results/numOfPredLabels_kNN.csv', numOfTrueLabels, delimiter=",")
...
plt.savefig('Figures/kNNGenrePredictor.png')
plt.clf()
plt.close()
...
for myK in range(0,num_of_ks):
    ...
    plt.title(myTitle)
    myFigName = 'Figures/JaccIndxHistforK_' + str(myK+1) + '.png'
...
for myK in range(0,num_of_ks):
    ...
    plt.title(myTitle)
    myFigName = 'Figures/JaccIndxVsTrueLabelsforK_' + str(myK+1) + '.png'
    plt.savefig(myFigName)
    plt.clf()
    plt.close()
...
for myK in range(0,num_of_ks):
    ...
    plt.title(myTitle)
    myFigName = 'Figures/TrueVsPredLabelsforK_' + str(myK+1) + '.png'
    plt.savefig(myFigName)
    plt.clf()
    plt.close()
